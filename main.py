...
class Game:
    ...
    def mostrar_logo_inicio(self,x,y):
        logo_inicio = self.tela_inicio.get_rect()
        logo_inicio.midtop = (x,y)
        self.tela.blit(self.tela_inicio,logo_inicio)


    def mostrar_tela_inicio(self):
        self.mostrar_logo_inicio(constantes.LARGURA/2,0)
        self.mostrar_texto('Se está preparado, pressione ENTER',40,constantes.BRANCO,constantes.LARGURA/2,constantes.ALTURA * 0.5)
        pygame.display.flip()
        esperando=True
        while esperando:
            self.clock.tick(constantes.FPS)
...
class Personagem(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.diretorio_arquivos = os.path.join(os.getcwd(),'assets')
        self.diretorio_imagens = os.path.join(self.diretorio_arquivos,"imagens")
        self.player = os.path.join(self.diretorio_imagens,constantes.IMAGEM_JOGADOR)
